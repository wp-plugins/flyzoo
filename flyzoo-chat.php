<?php
             ...
             */
             ...
             function flyzoo_chat_uninstall() {
                ...
                if(get_option('FlyzooHideInDashboard')) {  delete_option( 'FlyzooHideInDashboard'); }
    ...
    add_shortcode('flyzoo-embed-chatroom', 'flyzoo_embed_chatroom');
            ...
            class FlyzooWidget {
                        ...
                        if (get_option('FlyzooHideInDashboard')!=true) { 
                           $Path = $_SERVER['REQUEST_URI'];
                              ...
                              if (strpos($Path,'wp-admin/plugin-install.php?tab=plugin-information') === false) { 
                                    add_action('admin_footer', array($this, 'embedFlyzoo'));
                                    }
                               }

                        if(get_option('FlyzooApplicationID') =='') { $this->addNewWebsite(); }
                ...
                public function adminMenu() {
                        ...
                        add_menu_page('Flyzoo Live Chat', 'Flyzoo Live Chat', 'manage_options', 
                       'flyzoo-chat', array($this, 'createAdminPage'), content_url() . '/plugins/flyzoo/images/flyzoo-icon.png');
    
                }
    
                public function getSignupUrl(){
    
                    return $this->flyzooroot.'signup?utm_source=wordpress&utm_medium=admin&s=pro&t=12&fzsiteurl='.urlencode(site_url()).
                    '&p=wordpress&e='.urlencode(get_option('admin_email')).'&sip='.$_SERVER['REMOTE_ADDR'].
